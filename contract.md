...
6. **Auth resolution.** Reads `MORPHLLM_API_KEY` from env, or accepts `--api-key <key>` (flag overrides env). If both are absent, falls back to Doppler using `--doppler-token` / `DOPPLER_TOKEN`, `--doppler-project` / `WG_DOPPLER_PROJECT` (default `wg`), and `--doppler-config` / `WG_DOPPLER_CONFIG` (default `prd`). If all sources are absent or fail, prints a clear error to stderr and exits non-zero.

7. **Output modes.** Default mode prints `=== <path> ===` headers followed by content; `--json` mode prints the raw result as JSON to stdout. `--stream` mode prints `[turn N] <toolName>(args)` lines as the agent works.

8. **Typecheck clean.** `bunx tsc --noEmit` exits 0 against the existing `tsconfig.json`.

9. **Tests and lint.** `bun test` passes. `biome check .` passes. Auth and Doppler flag-wiring constraints are covered in `test/`.
